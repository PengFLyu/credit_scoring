...
def var_bins_output(df, selected_var, target):
    """
    Output binning results for selected variables
    """
    empty_bins = pd.DataFrame()

    for i in list(selected_var.keys()):
        var_bins = variable_binning(df, target, i, selected_var.get(i), 'bins')
        var_bins['var_name'] = i
        empty_bins = empty_bins.append(var_bins)
        
    return empty_bins


def KS_group(y, y_pred, bin_num=20):
    '''
    Calculate KS
    '''
    df = pd.DataFrame()
...
def standard_scoring(df, keep_list, target, selected_var,
            model = None, P = 660, PDO = 50, theta = 1/20):
    """
    Transformation from probabilities to standard scoring.
    """
    score_df = pd.DataFrame()
    score_df[keep_list] = df[keep_list]
    
    score_df['Y_prob'] = model.predict(df)
    
    B = PDO / math.log(2, math.e)
    A = P + B * math.log(theta, math.e)
    
    score_df['odds'] = (score_df['Y_prob'] / (1 - score_df['Y_prob']) ).apply(lambda x: math.log(x, math.e))
    score_df['score'] = round(A - B * score_df['odds'], 0)
    
    return score_df


def var_bins_output2(df, selected_var, target, 
                     model = None, P = 660, PDO = 50, theta = 1/20):
    """
    Output binning results from selected variables
    """
    ...
    for i in list(selected_var.keys()):
        var_bins = variable_binning(df, target, i, selected_var.get(i), 'bins')
        var_bins['var_name'] = i
        var_bins['score'] = round(((A - B * params['constant'])/n) - var_bins['WOE'] * B * params[i], 0)
        empty_bins = empty_bins.append(var_bins)
